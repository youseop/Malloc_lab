...
#define POINTER_PUT(p, val)    (*(unsigned int *)(p) = (int)(val)) 
...
static char* next_listp; //save next pointer
...
void link_free_list(void){
    char * bp = heap_listp;
    char* init_bp;
    size_t size = 0;
    while(0 != GET_SIZE(HDRP(bp))){
        if (GET_ALLOC(HDRP(bp)) == 0){
            ...
            PUT(HDRP(init_bp), PACK(size,0));
            PUT(FTRP(init_bp), PACK(size,0));
        }
        bp = NEXT_BLKP(bp);
    }
}
